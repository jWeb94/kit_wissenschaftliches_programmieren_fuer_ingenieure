...
Vector::Vector( const Vector & c ){
  ...
  //kopiere komponentenweise den Vektor c  (oder verwende memcopy)
...
Vector & Vector::operator= ( const Vector & c ){
  ...
  // Anzahl der Elemente nicht gleich: Feld anpassen (Luxusfunktion...)
  if(this->N != c.N){
    
      delete[] this->dataPtr;
      // try to allocate memory and catch an exception: wir gehen davon aus, dass das gut geht
      this->N = c.N;
...
const double  Vector::operator() ( std::size_t const n ) const{
...
double & Vector::operator() ( std::size_t const n ){
  // index check with assert: index check takes time 
  assert(n<this->N) ;
